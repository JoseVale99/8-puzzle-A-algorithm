...
class Puzzle:
    def __init__(self,textArea, no_nodos_exp, no_nodos_gen, costo)->None:
        self.textArea = textArea
        self.no_nodos_exp = no_nodos_exp
        self.no_nodos_gen = no_nodos_gen
        self.costo = costo
        self.print_final = ""
        self.values = ""
        self.array_print = ""
    ...
    def print_state(self,node):
        ...
        self.values = f""" g(n) =  {node.get_gn()}  h(n) =  {node.get_hn()} f(n) = {node.get_fn()} \n""" 
        self.array_print = f"""|   {node.get_current_state()[0]}   |   {node.get_current_state()[1]}   |{node.get_current_state()[2]}   |\n""" 
        self.array_print2 = f"""|  {node.get_current_state()[3]}   |   {node.get_current_state()[4]}   | {node.get_current_state()[5]}   |\n""" 
        self.array_print3 = f"""|  {node.get_current_state()[6]}   |   {node.get_current_state()[7]}   | {node.get_current_state()[8]}   |\n""" 
        self.line = "_________________\n"
        self.print_final += '\n Paso No: '+str(self.score)+'\n'+self.values+self.array_print+self.line+self.array_print2+self.line+self.array_print3+self.line +'########################################\n'

      
    def goal_reached(self,explored_nodes, count):
        ...
        for i in path:
            self.print_state(i)
        self.textArea.setPlainText(self.print_final)
        # print("Goal Reached \n")
        # print("The number of nodes expanded: " , nodes_expanded, "\n")
        # print("The number of nodes generated: ", count, "\n");
        # print("Path Cost: ", len(path)-1, "\n");

        self.no_nodos_exp.setText(str(nodes_expanded))
        self.no_nodos_gen.setText(str(count))
        self.costo.setText(str(len(path)-1))
